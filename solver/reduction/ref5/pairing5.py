...
def _find_best_setup(a: Coord, b: Coord, cap: int = 8) -> Optional[List[str]]:
    ...
    for goal in ((_FR_BOTTOM, _BR_TOP), (_BR_TOP, _FR_BOTTOM)):
        ...
        if best is None or len(seq) < len(best):
            best = seq
    _SETUP_CACHE[key] = tuple(best) if best is not None else None
    return best
...
def _wing_fp(cube) -> tuple:
    return tuple(edge_type_of_cubie(cube.cubies[p])
                 if cube.cubies.get(p) is not None else None for p in WINGS)


def pair_edges_beam(cube, beam_width: int = 6, per_state: int = 6,
                    max_swaps: int = 40) -> List[str]:
    """束搜索配棱：以「已配槽数」优先、压缩后动作数最少为目标。不修改输入。"""
    start = cube.clone()
    beam: List[Tuple[int, int, object, List[str]]] = [
        (12 * 9, -matched_slots(start), start, [])]
    best_log: Optional[List[str]] = None
    for _ in range(max_swaps):
        nxt: List[Tuple[int, int, object, List[str]]] = []
        done = False
        for _sc, _neg, w, log in beam:
            if edges_paired(w):
                done = True
                if best_log is None or len(log) < len(best_log):
                    best_log = log
                continue
            for _g, _l, _s, setup, ypos, other in _candidate_swaps(w, log)[:per_state]:
                w2 = w.clone()
                log2 = list(log)
                if not _swap_positions(w2, ypos, other, log2, setup=setup):
                    continue
                log2[:] = _compress_log(log2)
                pc2 = matched_slots(w2)
                nxt.append((len(log2) + (12 - pc2) * 9, -pc2, w2, log2))
        if done or not nxt:
            break
        ...
        for it in nxt:
            ...
            if len(beam) >= beam_width:
                break
    return best_log if best_log is not None else []
