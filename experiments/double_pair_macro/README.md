...
## 补充验证（2026-09-03）：单交换模型的尾段已到最优 —— 搜索侧无剩余余量

在贪心路径 matched≥7 的截点做**尾段精确最优解**（DFS + 分支定界，修正
下界为 `ceil(remaining/2)*9`，因为单个交换一次可净增 matched 至多 2；
memo 键 = 翼排列 + 日志尾），与 greedy / beam(width16, 2500 节点) 对比：

| id | unmatched | greedy | beam | exact |
|---|---|---|---|---|
| 0 | 4 | 102 | 100 | 100 |
| 1 | 5 | 124 | 124 | 124 |
| 2 | 4 | 111 | 107 | 107 |
...
结论：**beam 已达成精确最优（6/6 相等）**；贪心尾段仅高 0~4。因此：
- 单交换/P 架构下，任何“更聪明地搜顺序/加宽 beam/加双配”都无法再降——已到底；
- 要再降配棱步数，只能换**原始子集合**（真·非单调/结构性宏，或允许工作切片），
  属于结构性改造，收益不确定且工程量大；
- 若坚持“不推翻架构 + 保中心”，当前 ~95（seed-42）已接近该模型的理论下限。
